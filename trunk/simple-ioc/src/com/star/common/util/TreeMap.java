...
public class TreeMap extends LinkedHashMap<String, Node> {
     ...
     * @return this对象本身
     */
    public synchronized Map<String, Node> load(Class location, String... filePaths) {
    ...
    private void load(Class location, String filePath) {
        ...
        try {
            if (filePath.startsWith(FILE_PREFIX)) {
                in = new FileInputStream(filePath.substring(FILE_PREFIX.length()));
            }
                ...
                in = Thread.currentThread().getContextClassLoader().getResourceAsStream(
                        filePath.charAt(0) == '/' ? filePath.substring(1) : filePath);
            }
            if (in == null) {
                throw new RuntimeException("can't read file[" + filePath + "] from ClassLocation["
                        + location + "]");
            }
            load(in);
        }
    ...
    public synchronized Node put(String key, Node node) {
        ...
        if (lastLay == 0) {
            if (containsKey(key)) {
                System.err.println("Node(key[" + key + "]) has exist, old node is replaced.");
            }
    ...
    private void doLine(Node node) {
        ...
        if (lay == 0) {
            ...
            throw new IllegalArgumentException("lay error. lay=" + lay + ",but lastLay=" + lastLay
                    + ", node=" + node + ", lastNode=" + lastNode);
        }
        s.push(node);
        lastLay = lay;
        lastNode = node;
    ...
    private Node nodeInject(String key, String value, int index) {
        ...
        for (int j = 0; j < parentKeys.length; j++) {
            if (j == 0) {
                ...
                if (parent == null) {
                    throw new IllegalArgumentException("can't find node: " + parentKeys[j - 1]
                            + ", for Node = {key:" + key + ", value:" + value + "}.");
                }
